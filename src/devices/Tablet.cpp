...
CTablet::CTablet(wlr_tablet* tablet_) : tablet(tablet_) {
    ...
    Debug::log(LOG, "[tbdebug] Tablet new: {:x}", (uintptr_t)this);
    ...
    hyprListener_button.initCallback(&tablet->events.button, [this] (void* owner, void* data) {
        ...
        Debug::log(LOG, "[tbdebug] Tablet wlr event for {:x} with tool {:x} (button {} state {} time {})", (uintptr_t)self.get(), (uintptr_t)E->tool, E->button, E->state == WLR_BUTTON_PRESSED, E->time_msec);
