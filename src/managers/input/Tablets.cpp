...
static void refocusTablet(SP<CTablet> tab, SP<CTabletTool> tool) {
...
void CInputManager::onTabletAxis(CTablet::SAxisEvent e) {
    ...
    switch (e.tool->type) {
        case WLR_TABLET_TOOL_TYPE_MOUSE:
            g_pPointerManager->move(e.axisDelta);
...
void CInputManager::onTabletButton(CTablet::SButtonEvent e) {
    ...
    Debug::log(LOG, "[tbdebug] Tablet event for {:x} with tool {:x} (button {} state {} time {})", (uintptr_t)e.tablet.get(), (uintptr_t)PTOOL.get(), e.button, e.down, e.timeMs);
...
SP<CTabletTool> CInputManager::ensureTabletToolPresent(wlr_tablet_tool* pTool) {
    if (pTool->data == nullptr) {
        ...
        Debug::log(LOG, "[tbdebug] Ensure tool new: {:x} for wlr {:x}", (uintptr_t)PTOOL.get(), (uintptr_t)pTool);

        Debug::log(LOG, "[tbdebug] Creating tablet tool v2 for {:x}", (uintptr_t)pTool);
        ...
        Debug::log(LOG, "[tbdebug] Ensure tool already present for: {:x}", (uintptr_t)pTool);
    }
