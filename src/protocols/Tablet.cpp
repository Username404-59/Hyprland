...
CTabletToolV2Resource::~CTabletToolV2Resource() {
    if (frameTimer)
        g_pEventLoopManager->removeTimer(frameTimer);
}
...
void CTabletToolV2Resource::queueFrame() {
    ...
    frameTimer = makeShared<CEventLoopTimer>(
        std::chrono::milliseconds(1), [this](SP<CEventLoopTimer> self, void* data) { sendFrame(); }, nullptr);
    g_pEventLoopManager->addTimer(frameTimer);
}
...
void CTabletV2Protocol::proximityIn(SP<CTabletTool> tool, SP<CTablet> tablet, wlr_surface* surf) {
    ...
    for (auto& t : m_vTools) {
        ...
        LOGM(LOG, "proximityIn: tabs {}", m_vTablets.size());

        SP<CTabletV2Resource> tabletResource;
        for (auto& tab : m_vTablets) {
            LOGM(LOG, "proximityIn: d1: tab {:x}", (uintptr_t)tab->tablet.get());
            if (tab->tablet != tablet)
                continue;

            LOGM(LOG, "proximityIn: d2: tab {:x} {:x}", (uintptr_t)tab->seat.get(), (uintptr_t)t->seat.get());
        ...
        if (!tabletResource) {
            LOGM(ERR, "proximityIn on a tool with a tablet that does not have a resource??");
            return;
        }

        LOGM(LOG, "[tbdebug] proximity in for {:x}", (uintptr_t)tool.get());

        t->current  = true;
        t->lastSurf = surf;

        auto serial = wlr_seat_client_next_serial(wlr_seat_client_for_wl_client(g_pCompositor->m_sSeat.seat, t->resource->client()));
        t->resource->sendProximityIn(serial, tabletResource->resource.get(), surf->resource);
        t->queueFrame();
...
void CTabletV2Protocol::proximityOut(SP<CTabletTool> tool) {
    for (auto& t : m_vTools) {
        ...
        LOGM(LOG, "[tbdebug] proximity out for {:x}", (uintptr_t)tool.get());
...
void CTabletV2Protocol::buttonTool(SP<CTabletTool> tool, uint32_t button, uint32_t state) {
    LOGM(LOG, "[tbdebug] buttonTool: tool {:x}, button {} state {}. Registered resources: {}", (uintptr_t)tool.get(), button, state, m_vTools.size());

    for (auto& t : m_vTools) {
        if (t->tool != tool || !t->current)
            continue;

        LOGM(LOG, "[tbdebug] buttonTool: tool {:x} passed for resource {:x}", (uintptr_t)tool.get(), (uintptr_t)t->resource.get());
